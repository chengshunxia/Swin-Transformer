...
def get_data(args, opts, train=True, async_dataloader=False, return_remaining=False, fine_tuning=False):
    ...
    input_shape = (3, args.DATA.IMG_SIZE, args.DATA.IMG_SIZE)
    if args.IPU.precision[:3] == "16.":
        half_precision = True
    elif args.IPU.precision[:3] == "32.":
        half_precision = False
    use_bbox_info = getattr(args.IPU, "use_bbox_info", False)

    if args.IPU.data in ["real", "imagenet"]:
        transform = get_preprocessing_pipeline(train, input_shape[-1],
                                               half_precision, args.IPU.normalization_location == "host", eightbit = args.IPU.eight_bit_io,
                                               use_bbox_info=use_bbox_info, fine_tuning=fine_tuning)
    # Determine the size of the small datasets
    if hasattr(args.IPU, "iterations"):
        dataset_size = args.DATA.BATCH_SIZE * \
    ...
    if args.IPU.data in ["synthetic", "generated"]:
        if hasattr(args.IPU, "iterations"):
            dataset = GeneratedDataset(input_shape, size=dataset_size, half_precision=half_precision, eightbit=args.IPU.eight_bit_io)
        else:
            dataset = GeneratedDataset(input_shape, half_precision=half_precision, eightbit=args.IPU.eight_bit_io)
    elif args.IPU.data == "real":
    ...
    elif args.IPU.data == "imagenet":
        assert os.path.exists(args.DATA.DATA_PATH), f"{args.DATA.DATA_PATH} does not exist!"
        if os.path.exists(os.path.join(args.DATA.DATA_PATH, 'metadata.json')):
            # WebDataset format
            ...
            data_folder = os.path.join(args.DATA.DATA_PATH, data_folder)
            if os.path.exists(data_folder):
                # Original ImageNet format
                bboxes = os.path.join(args.DATA.DATA_PATH, 'imagenet_2012_bounding_boxes.csv') if use_bbox_info and train else None   # use bboxes only for training
    ...
    global_batch_size = args.DATA.BATCH_SIZE * opts.device_iterations * opts.replication_factor * opts.Training.gradient_accumulation
    if async_dataloader:
    ...
    worker_initialization = _WorkerInit(args.IPU.seed, opts.Distributed.processId, args.IPU.dataloader_worker) if hasattr(args.IPU, 'seed') else None
    rebatch_size = getattr(args.IPU, "dataloader_rebatch_size", None)
    ...
    dataloader = poptorch.DataLoader(opts,
                                     ...
                                     num_workers=args.IPU.dataloader_worker,
